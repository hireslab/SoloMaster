...
switch action
    ...
    case 'update'
        if existparam(me, 'RP')
            ...
            State   =       invoke(RP,'GetTagVal','State');
            EventCounter=   invoke(RP,'GetTagVal','EventCounter');
            Event       =   invoke(RP,'ReadTagVex','Event',LastEventCounter,EventCounter-LastEventCounter,'F32','F64',1);
            EventTime  =   invoke(RP,'ReadTagVex','EventTime',LastEventCounter,EventCounter-LastEventCounter,'F32','F64',1);
    ...
    case 'state35'
        ...
        TrialEvent       = invoke(RP,'ReadTagVex','Event',LastTrialEventCounter,n_events,'F32','F64',1);
        TrialEventTimer  = invoke(RP,'ReadTagVex','EventTime',LastTrialEventCounter,n_events,'F32','F64',1);
        tevent = [];
        event_n=1;
        for i=1:n_events
...
%         SaveParamsTrial(me);
        ...
        invoke(RP,'SoftTrg',1);    % Trigger to generate a timeup event to go back to state 0
        ...
        for i=1:length(val)
            if RPBitsOut(bit(i)+1)>=0
                ...
                if existparam('rpbox', 'RP')
                    ...
                    invoke(RP,'SetTagVal','Bits_HighVal',Bits_HighVal);
                    invoke(RP,'SoftTrg',6);            
                end
            end
        end
    ...
    case 'ao_out'
        ...
        if nargin <2 
            ...
            % called from a function
            if nargin < 3
                val = varargin{2}; AO = find(ones(size(val))); % all bits if no bit was specified
            else
                AO  = varargin{2}; val = varargin{3};
            end
        end
        
        OldRP_AO_Out  =GetParam(me,'RP_AO_Out');  RP_AO_Out = OldRP_AO_Out;
        RP_AO_Out(AO) = val;
        if RP_AO_Out(1), RP_AO_Out(3)=0; end;  % Here we enforce some bit rules, because 1 and 3
        % share the same analog channel;  
        ...
        for i=1:3
            ...
            % set the AO_Out
            AOBits_HighVal=bin2dec(sprintf('%d%d%d%d',RP_AO_Out(3:-1:1).*RP_AO_Out(3:-1:1)>0));
            if existparam('rpbox', 'RP')
                RP=GetParam('rpbox', 'RP');
                invoke(RP,'SetTagVal','AOBits_HighVal',AOBits_HighVal);
                invoke(RP,'SoftTrg',8); 
            end
        end
...
invoke(RP,'WriteTagV','StateMatrix',0,state_addr);
...
if existparam(me, 'RP')
    ...
    if ~isempty(RP)
        if existparam(me, 'DefID')
            ...
            while invoke(RP,'DefStatus',DefID)>0  %% wait for def write 
...
RP=actxcontrol('RPco.x',[20 20 60 60],RPh);
...
invoke(RP,'Halt');
invoke(RP,'ClearCOF');
if invoke(RP,'ConnectRP2','GB',1)         %default use RP2
    if ~invoke(RP,'LoadCOF',[GetParam('rpbox','protocol_path') '\RP2Box.rco']);
...
elseif invoke(RP,'ConnectRP2','USB',1)     % connect to RP2 using 'USB' method
    if ~invoke(RP,'LoadCOF',[GetParam('rpbox','protocol_path') '\RP2Box.rco']);
        Message(me, 'LoadCOF Failure');
        error('failed to load RP2 control object file. Power cycle RP2 and try again.')
    else
...
elseif invoke(RP,'ConnectRM1','USB',1)     % connect to RM1 using 'USB' method
    if ~invoke(RP,'LoadCOF',[GetParam('rpbox','protocol_path') '\RM1Box.rco']);
        Message(me, 'LoadCOF Failure');
...
RPS=actxcontrol('RPco.x',[20 20 60 60],RPSh);
...
% invoke(RPS,'Halt');
% invoke(RPS,'ClearCOF');
if invoke(RPS,'ConnectRP2','GB',2)         %default use second RP2
    if ~invoke(RPS,'LoadCOF',[GetParam('rpbox','protocol_path') '\2SoundRP2_2.rco']);
        Message(me, 'LoadCOF Failure');
        error('failed to load RP2 control object file. Power cycle RP2 and try again.')
    else
...
elseif invoke(RPS,'ConnectRP2','USB',2)     %connect to RP2 using 'USB' method
    if ~invoke(RPS,'LoadCOF',[GetParam('rpbox','protocol_path') '\2SoundRP2_2.rco']);
        Message(me, 'LoadCOF Failure');
        error('failed to load RP2 control object file. Power cycle RP2 and try again.')
    else
...
elseif invoke(RPS,'ConnectRM1','USB',2)     %connect to RM1 using 'USB' method
    if ~invoke(RPS,'LoadCOF',[GetParam('rpbox','protocol_path') '\2SoundRM1_2.rco']);
        Message(me, 'LoadCOF Failure');
        error('failed to load RM1 control object file. Power cycle RM1 and try again.')
    else
...
% invoke(RPS,'SoftTrg',6);    %disable trigger to prevent unwanted trigger
invoke(RPS,'SoftTrg',2);    % Stop and Reset
...
for i=1:length(id) 
	if RP2_ready
        datain_chk=invoke(RPS,'WriteTagV',['datain' num2str(id(i))],0,beep{id(i)}*1); %when use with regular speaker
%         datain_chk=invoke(RPS,'WriteTagV',['datain' num2str(i)],0,beep{i}*10); %when use with sound amplifier
	else
        datain_chk=invoke(RPS,'WriteTagV',['datain' num2str(id(i))],0,beep{id(i)});
	end
	datalngth_chk=invoke(RPS,'SetTagVal',['datalngth' num2str(id(i))],length(beep{id(i)}));
end
...
RPSS=actxcontrol('RPco.x',[20 20 60 60],RPSSh);
...
% invoke(RPSS,'Halt');
% invoke(RPSS,'ClearCOF');
if invoke(RPSS,'ConnectRP2','GB',2)         %default use second RP2
    if ~invoke(RPSS,'LoadCOF',[GetParam('rpbox','protocol_path') '\SSoundRP2_2.rco']);
        Message(me, 'LoadCOF Failure');
        error('failed to load RP2 control object file. Power cycle RP2 and try again.')
    else
...
elseif invoke(RPSS,'ConnectRP2','USB',2)     %connect to RP2 using 'USB' method
    if ~invoke(RPSS,'LoadCOF',[GetParam('rpbox','protocol_path') '\SSoundRP2_2.rco']);
        Message(me, 'LoadCOF Failure');
        error('failed to load RP2 control object file. Power cycle RP2 and try again.')
    else
...
elseif invoke(RPSS,'ConnectRM1','USB',2)     %connect to RM1 using 'USB' method
    if ~invoke(RPSS,'LoadCOF',[GetParam('rpbox','protocol_path') '\SSoundRM1_2.rco']);
        Message(me, 'LoadCOF Failure');
        error('failed to load RM1 control object file. Power cycle RM1 and try again.')
    else
...
invoke(RPSS,'SoftTrg',2);    % Stop and Reset
invoke(RPSS,'SoftTrg',4);    % Stop and Reset
...
if strcmp(GetParam('rpbox','RPSSDevice'),'RP2')
    invoke(RPSS,'WriteTagV','datain1',0,beep{1}*1); %when use with regular speaker
    invoke(RPSS,'WriteTagV','datain2',0,beep{2}*1); %when use with regular speaker
    invoke(RPSS,'WriteTagV','datain3',0,beep{3}*1); %when use with regular speaker
    invoke(RPSS,'WriteTagV','datain4',0,beep{4}*1); %when use with regular speaker
%     invoke(RPSS,'WriteTagV','datain1',0,beep{1}*10); %when use with sound amplifier
%     invoke(RPSS,'WriteTagV','datain2',0,beep{2}*10); %when use with sound amplifier
%     invoke(RPSS,'WriteTagV','datain3',0,beep{3}*10); %when use with sound amplifier
%     invoke(RPSS,'WriteTagV','datain4',0,beep{4}*10); %when use with sound amplifier
else
    invoke(RPSS,'WriteTagV','datain1',0,beep{1});
    invoke(RPSS,'WriteTagV','datain2',0,beep{2});
    invoke(RPSS,'WriteTagV','datain3',0,beep{3});
    invoke(RPSS,'WriteTagV','datain4',0,beep{4});
end
invoke(RPSS,'SetTagVal','datalngth1',length(beep{1}));
invoke(RPSS,'SetTagVal','datalngth2',length(beep{2}));
invoke(RPSS,'SetTagVal','datalngth3',length(beep{3}));
invoke(RPSS,'SetTagVal','datalngth4',length(beep{4}));
